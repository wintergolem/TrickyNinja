...
/////// Normals:
                float2 node_2384 = i.uv0;
                float3 normalLocal = (UnpackNormal(tex2D(_Rockm11111ormal,TRANSFORM_TEX(node_2384.rg, _Rockm11111ormal))).rgb*1.0);
...
////// Emissive:
                float3 node_1503 = (tex2D(_Diffuse,TRANSFORM_TEX(node_2384.rg, _Diffuse)).rgb*_DiffuseTint.rgb);
                float4 node_2383 = _Time + _TimeEditor;
                float2 node_482 = (i.uv0.rg+node_2383.g*float2(0.03,-0.03));
                float2 node_845 = (i.uv0.rg+node_2383.g*float2(-0.02,0.02)); // Movement is set using the U and V fields below. Positive and Negative values control direction Values themselves are the speed.
                float4 node_2241 = i.vertexColor;
                float3 node_2257 = lerp(node_1503,lerp(((tex2D(_RotationLayer2,TRANSFORM_TEX(node_482, _RotationLayer2)).rgb*pow(_EmissionColor.rgb,2.0))+(tex2D(_RotationLayer,TRANSFORM_TEX(node_845, _RotationLayer)).rgb*pow(_Rotation1Color.rgb,1.0))),node_1503,tex2D(_StationaryLayerMask,TRANSFORM_TEX(node_2384.rg, _StationaryLayerMask)).a),node_2241.r);
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = tex2Dlod(_RockSpecular,float4(TRANSFORM_TEX(node_2384.rg, _RockSpecular),0.0,_node_2068)).rgb;
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                ...
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
/////// Normals:
                float2 node_2386 = i.uv0;
                float3 normalLocal = (UnpackNormal(tex2D(_Rockm11111ormal,TRANSFORM_TEX(node_2386.rg, _Rockm11111ormal))).rgb*1.0);
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = tex2Dlod(_RockSpecular,float4(TRANSFORM_TEX(node_2386.rg, _RockSpecular),0.0,_node_2068)).rgb;
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                float3 node_1503 = (tex2D(_Diffuse,TRANSFORM_TEX(node_2386.rg, _Diffuse)).rgb*_DiffuseTint.rgb);
                float4 node_2385 = _Time + _TimeEditor;
                float2 node_482 = (i.uv0.rg+node_2385.g*float2(0.03,-0.03));
                float2 node_845 = (i.uv0.rg+node_2385.g*float2(-0.02,0.02)); // Movement is set using the U and V fields below. Positive and Negative values control direction Values themselves are the speed.
                float4 node_2241 = i.vertexColor;
                float3 node_2257 = lerp(node_1503,lerp(((tex2D(_RotationLayer2,TRANSFORM_TEX(node_482, _RotationLayer2)).rgb*pow(_EmissionColor.rgb,2.0))+(tex2D(_RotationLayer,TRANSFORM_TEX(node_845, _RotationLayer)).rgb*pow(_Rotation1Color.rgb,1.0))),node_1503,tex2D(_StationaryLayerMask,TRANSFORM_TEX(node_2386.rg, _StationaryLayerMask)).a),node_2241.r);
                finalColor += diffuseLight * node_2257;
                finalColor += specular;
/// Final Color:
