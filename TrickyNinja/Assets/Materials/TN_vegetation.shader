...
/////// Normals:
                ...
                float3 normalLocal = tex2D(_Normal,TRANSFORM_TEX(node_1826.rg, _Normal)).rgb;
...
////// Specular:
                ...
                float3 specularColor = (pow(max(0,dot(node_7,halfDirection)),exp2((tex2D(_Gloss,TRANSFORM_TEX(node_1826.rg, _Gloss)).rgb*_Glossiness)))*(_Specularity*tex2D(_Specular,TRANSFORM_TEX(node_1826.rg, _Specular)).rgb));
...
/// Final Color:
        ...
        Pass {
            ...
            fixed4 frag(VertexOutput i) : COLOR {
                i.normalDir = normalize(i.normalDir);
                float2 node_1828 = i.uv0;
                float4 node_10 = tex2D(_Diffuse,TRANSFORM_TEX(node_1828.rg, _Diffuse));
                clip(node_10.a - 0.5);
