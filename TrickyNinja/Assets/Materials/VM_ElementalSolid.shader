...
/////// Normals:
                float2 node_388 = i.uv0;
                float3 normalLocal = tex2D(_RockNormal,TRANSFORM_TEX(node_388.rg, _RockNormal)).rgb;
...
/////// Diffuse:
                ...
                #endif
////// Emissive:
                float4 node_102 = tex2D(_RedGreenChannelMask,TRANSFORM_TEX(node_388.rg, _RedGreenChannelMask));
                float3 node_21 = (tex2D(_Layer3,TRANSFORM_TEX(node_388.rg, _Layer3)).rgb*_Layer3Color.rgb*node_102.g);
                float4 node_387 = _Time + _TimeEditor;
                float2 node_4 = (i.uv0.rg+node_387.g*float2(-0.1,0.1));
                float2 node_12 = (i.uv0.rg+node_387.g*float2(0.1,-0.1));
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = (tex2D(_Specular,TRANSFORM_TEX(node_388.rg, _Specular)).rgb*_SpecularValue);
                ...
                diffuseLight += (tex2D(_DiffuseAmbientight,TRANSFORM_TEX(node_388.rg, _DiffuseAmbientight)).rgb*_DiffuseAmbLightColor.rgb); // Diffuse Ambient Light
                #ifdef LIGHTMAP_OFF
...
/////// Normals:
                ...
                float3 normalLocal = tex2D(_RockNormal,TRANSFORM_TEX(node_390.rg, _RockNormal)).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
////// Specular:
                NdotL = max(0.0, NdotL);
                float3 specularColor = (tex2D(_Specular,TRANSFORM_TEX(node_390.rg, _Specular)).rgb*_SpecularValue);
                float3 specular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor;
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                float4 node_102 = tex2D(_RedGreenChannelMask,TRANSFORM_TEX(node_390.rg, _RedGreenChannelMask));
                float3 node_21 = (tex2D(_Layer3,TRANSFORM_TEX(node_390.rg, _Layer3)).rgb*_Layer3Color.rgb*node_102.g);
                finalColor += diffuseLight * node_21;
                finalColor += specular;
/// Final Color:
