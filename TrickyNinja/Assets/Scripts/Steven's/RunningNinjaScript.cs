...
//Handles the running ninja enemy. The guy that runs at you then attacks
//based off monkscript, stripped down

using UnityEngine;
using System.Collections;

public class RunningNinjaScript : EnemyScript {
	...
	void Update () 
	{
		if(!bDead)
		{
			...
			if(!bDie && fCurAttackTime > 0)
			{
				if( Mathf.Abs( gPlayer.transform.position.x - transform.position.x ) < fHitDistance )
				{
					CustomAttack();
				}
				else
